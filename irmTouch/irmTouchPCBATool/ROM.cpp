...
// ---------------------------------------------------------------------------
__fastcall TFormROM::TFormROM(TComponent* Owner) : TForm(Owner) {
}
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
void __fastcall TFormROM::ShowRomData(TStringGrid *StringGrid,
	unsigned char *Data, unsigned len, unsigned StartAdd) {
	int i;
	String Msg;
	...
	for (unsigned j = 0; j < (len + 15) / 16; j += 1) {
		...
		for (i = 1; i < 9; i += 1) {
			StringGrid->Cells[i][j + 1] =
				Msg.sprintf(L"%.4X", RomBuf[j * 8 + i - 1]);
		}
	}
}

void __fastcall TFormROM::FormShow(TObject *Sender) {
	ShowRomData(StringGrid1, UpdateCtrl.MainFlash + UpdateCtrl.AppStartAdd,
		UpdateCtrl.AppSize, UpdateCtrl.AppStartAdd);
