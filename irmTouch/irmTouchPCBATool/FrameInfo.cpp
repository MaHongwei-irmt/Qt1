...
// ----------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------
void __fastcall TouchFrame::UpdateFrameInfo(DebugHead_t *dh,
	...
	if (it->YLeds == 0) {
		YLen = Y_MARGIN_TOTAL + it->XLeds * LED_SPACE * 9 / 16;
		// fix me!!!!!!!!! 16:9
	}
	else {
	...
	if (it->YLeds > 0) {
		if (MAX_X_OFF + MAX_Y_OFF > 0) {
			it->XCornerAxis = it->YCornerAxis =
				CornerSignals / (MAX_X_OFF + MAX_Y_OFF);
		}
		else {
...
// ----------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------------------
void __fastcall TouchFrame::UpdateFrameAuxInfo(DebugHeadV2_t *dh,
	uint16_t CornerSignals) {
