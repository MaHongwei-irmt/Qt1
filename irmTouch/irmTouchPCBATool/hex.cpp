...
int __fastcall HEXFile::GetFileData(String File) {
	...
	while (1) {
		...
		if (LineBuf[0] != ':') {
			...
			if (LineFormat == 0) {
				...
				if (UpdateCtrl.ROMStartAdd > (uint32_t)StartAdd +
					FLASH_BASE_ADD) {
					UpdateCtrl.ROMStartAdd = StartAdd + FLASH_BASE_ADD;
				}
				...
				if (StartAdd + LineDataLen <= len) {
					...
					if (UpdateCtrl.ROMEndAdd < StartAdd + LineDataLen +
						FLASH_BASE_ADD) {
						UpdateCtrl.ROMEndAdd =
							StartAdd + LineDataLen + FLASH_BASE_ADD;
					}
	...
	UpdateCtrl.FlashSize =
		(UpdateCtrl.AppEndAdd + FormUpdate->FlashParam.EndAreaSize) / 1024;
