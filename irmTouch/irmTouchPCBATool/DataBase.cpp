...
void __fastcall TFormViewDataBase::AddRecord(uint8_t *Data, uint32_t Index) {
	...
	// Time
	memcpy(&ShipTime, Ptr, sizeof(ShipTime));
	Ptr += sizeof(ShipTime);

	TimeDate = localtime(&ShipTime);
	Msg.printf(L"%d-%d-%d", TimeDate->tm_year + 1900, TimeDate->tm_mon + 1,
		TimeDate->tm_mday);
	...
	if (String(Ptr).Length() > 0) {
		...
		else if (StringGrid1->Cells[5][Index + 1] == "5.40" ||
			StringGrid1->Cells[5][Index + 1] == "6.50")
