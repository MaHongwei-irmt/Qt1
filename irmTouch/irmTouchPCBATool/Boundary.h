...
public: // User declarations
	...
	interval_t TotalIntervals[MAX_AXIS * 4 + MAX_CORNER_AXIS];
	// Intervals + AuxIntervals
	interval_t MixIntvs[MAX_AXIS * 4 + MAX_CORNER_AXIS];
	// Intervals + AuxIntervals
...
#define COMLEN_FACTOR 3 //类似于原detect_c.c中的F参数
	...
	void __fastcall FindSignalPit(uint8_t* norm, uint8_t normindex,
		uint8_t pitnum, int lednum);
	void __fastcall CopyPit(int lednum, uint8_t start, uint8_t end,
		uint8_t startsignal, uint8_t endsignal, uint8_t condition,
		uint8_t pitnum);
	void __fastcall Boundary::FindBoundaryX(void);
	void __fastcall Boundary::FindBoundaryY(void);
	void __fastcall Boundary::CopySideLine(uint8_t sidelinenum,
		unsigned int recx, unsigned int recy, unsigned int trmx,
		unsigned int trmy, bool isvalid);

	// function declarations for new D2 algorithm
	void __fastcall Boundary::getInterval1(void);
	void __fastcall Boundary::scanAdjacentMainX1(uint8_t axisnum,
		uint8_t axisindexsmall, uint8_t axisindexlarge, int16_t startled,
		int16_t endled);
	void __fastcall Boundary::scanAdjacentNegativeX1(uint8_t axisnum,
		uint8_t axisindexsmall, uint8_t axisindexlarge, int16_t startled,
		int16_t endled, uint8_t rightcorner);
	void __fastcall Boundary::scanAdjacentPositiveX1(uint8_t axisnum,
		uint8_t axisindexsmall, uint8_t axisindexlarge, int16_t startled,
		int16_t endled, uint8_t leftcorner);
	void __fastcall Boundary::scanAdjacentMainY1(uint8_t axisnum,
		uint8_t axisindexsmall, uint8_t axisindexlarge, int16_t startled,
		int16_t endled);
	void __fastcall Boundary::scanAdjacentNegativeY1(uint8_t axisnum,
		uint8_t axisindexsmall, uint8_t axisindexlarge, int16_t startled,
		int16_t endled, uint8_t upcorner);
	void __fastcall Boundary::scanAdjacentPositiveY1(uint8_t axisnum,
		uint8_t axisindexsmall, uint8_t axisindexlarge, int16_t startled,
		int16_t endled, uint8_t downcorner);
	void __fastcall Boundary::copyIntervalXF1(uint8_t axisnum,
		int16_t fallingedgerecx, int16_t offsetf);
	void __fastcall Boundary::copyIntervalXR1(uint8_t axisnum,
		int16_t risingedgerecx, int16_t offsetr);
	void __fastcall Boundary::copyIntervalYF1(uint8_t axisnum,
		int16_t fallingedgerecy, int16_t offsetf);
	void __fastcall Boundary::copyIntervalYR1(uint8_t axisnum,
		int16_t risingedgerecy, int16_t offsetr);
	void __fastcall Boundary::copyBlockedLineX1(uint8_t axisnum, int16_t recx,
		int16_t offset);
	void __fastcall Boundary::copyBlockedLineY1(uint8_t axisnum, int16_t recy,
		int16_t offset);
	// dynamic scan
