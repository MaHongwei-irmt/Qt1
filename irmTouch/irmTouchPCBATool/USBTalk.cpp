...
__fastcall USBTalk::USBTalk(bool CreateSuspended) : TThread(CreateSuspended) {
	Len = 0;
	Sta = IDLE;
	Exit = false;
	...
	sprintf(c, "d:\\logRX-%d.txt", ltime);

	if ((fpDebugMsg = fopen(c, "w")) == NULL) {
...
void __fastcall USBTalk::Execute() {
	...
	while (!Exit) {
		...
		if (Connected && (ReadHid(&DebugHidDev, Report, 64) == TRUE)) {
			memcpy(&DebugHidDev.Reports[DebugHidDev.ReportInPtr][0],
			Report, 64);
			DebugHidDev.ReportInPtr =
				(DebugHidDev.ReportInPtr + 1) & REPORT_FIFO_MASK;
			NoDelay = true;
		}
...
bool __fastcall USBTalk::SendUpdateFrame(void) {
	...
	if (Len != SendLen && Cancle == false) {
		...
		if (CurSendLen > (uint16_t)(1024 - (Head->StartAdd & 0x3FF)))
			// Across 1024Bytes Boundary
		{
		...
		if ((Head->Mode & MODE_ENCODE) != 0) {
			...
			Des_Go(Temp2, &Report[8], 48, this->Key, sizeof(this->Key),
				DECRYPT);
...
// Note:
// ---------------------------------------------------------------------------
bool __fastcall USBTalk::SubmitTask(uint8_t Cmd, uint32_t Len, uint8_t *Data,
	PROGRAM_HEAD *Head) {
	if (Sta != IDLE) {
		return false;
	}
...
BOOL USBTalk::EnumerateHidDevice(USHORT vid, USHORT pid) {
	HANDLE HidDevHandle;
	static GUID HidGuid;
	/* HID Globally Unique ID: windows supplies us with this value */
	HDEVINFO HidDevInfo;
	/* handle to structure containing all attached HID Device information */
	...
	EnumerateMsg.cat_printf(L"%.8X-%.4X-%.4X-", HidGuid.Data1, HidGuid.Data2,
		HidGuid.Data3);
	...
	for (int Index = 0; Index < 128; Index += 1) {
		...
		Result = SetupDiEnumDeviceInterfaces(HidDevInfo, 0, &HidGuid, Index,
			&devInfoData);
		...
		SetupDiGetDeviceInterfaceDetail(HidDevInfo, &devInfoData, NULL, 0,
			&DataSize, NULL);
		...
		SetupDiGetDeviceInterfaceDetail(HidDevInfo, &devInfoData, detailData,
			DataSize, &RequiredSize, NULL);
		...
		if (HidDevHandle != INVALID_HANDLE_VALUE) {
			...
			EnumerateMsg.cat_printf(L"%.4x:%.4x, ", HIDAttrib.VendorID,
				HIDAttrib.ProductID);
...
BOOL USBTalk::ReadHid(irmtouch_t *Dev, uint8_t *Data, uint8_t BufLen) {
	...
	/* get a temperature report from the HID device.  Note that
	 ...
	 on the IN endpoint. */
	bResult = ReadFile(Dev->Handle, /* handle to device */ &inbuffer[0],
		/* IN report buffer to fill */ Dev->Caps.InputReportByteLength,
		/* input buffer size */
		// 62,
		&numBytesReturned, /* returned buffer size */ (LPOVERLAPPED)
		& Dev->Overlapped); /* long pointer to an OVERLAPPED structure */
	...
	if (bResult) {
		...
		if (bResult == WAIT_TIMEOUT || bResult == WAIT_ABANDONED) {
			...
			GetOverlappedResult(Dev->Handle, &Dev->Overlapped,
				&numBytesReturned, TRUE);
...
// Note:
// ---------------------------------------------------------------------------
BOOL USBTalk::WriteHid(irmtouch_t *Dev, uint8_t *Data, uint8_t Len,
	uint32_t &Written) {
	BOOL bResult;
	...
	bResult = WriteFile(Dev->Handle, Data, Dev->Caps.OutputReportByteLength,
		(unsigned long *)&Written, (LPOVERLAPPED) & Dev->Overlapped);
	...
	if (bResult) {
		...
		if (bResult == WAIT_TIMEOUT || bResult == WAIT_ABANDONED) {
			...
			GetOverlappedResult(Dev->Handle, &Dev->Overlapped,
				(unsigned long *)&Written, TRUE);
		}
	}

	fprintf(fpDebugMsg, "->:(%d)", Dev->Caps.OutputReportByteLength);
	...
	for (uint32_t i = 0; i < Dev->Caps.OutputReportByteLength; i += 1) {
		fprintf(fpDebugMsg, "0x%.2X, ", Data[i]);
	}
	fprintf(fpDebugMsg, "\n\n");
	fflush(fpDebugMsg);
...
// ---------------------------------------------------------------------------
USHORT PIDVID[][5] = {
	{0x23c9, 0x5666, 1, 2, 63}, {0x23c9, 0x5667, 1, 2, 63},
	{0x23c9, 0x5668, 1, 2, 63},

	{0x000B, 0x3C00, 1, 2, 63}, {0x0717, 0x3C00, 1, 2, 63},
	{0x0368, 0x3C00, 1, 2, 63},

	{0x227d, 0x0709, 1, 2, 63}, {0x23c9, 0x5800, 1, 2, 63},
	{0x0B8C, 0x3200, 7, 7, 63}, {0x6100, 0x285A, 1, 2, 63},
	{0x285A, 0x6100, 1, 2, 63}, {0x04B4, 0xC001, 1, 2, 63}, {0, 0, 0, 0, 0}, };

// USHORT PIDVID[][5] = {
// {0x23c9, 0x5666, 1, 2, 62},
// {0x23c9, 0x5667, 1, 2, 62},
// {0x23c9, 0x5668, 1, 2, 62},
//
// {0x000B, 0x3C00, 1, 2, 62},
// {0x0717, 0x3C00, 1, 2, 62},
// {0x0368, 0x3C00, 1, 2, 62},
//
// {0x227d, 0x0709, 1, 2, 62},
// {0x23c9, 0x5800, 1, 2, 62},
// {0x0B8C, 0x3200, 7, 7, 62},
// {0x6100, 0x285A, 1, 2, 62},
// {0x285A, 0x6100, 1, 2, 62},
// {0x04B4, 0xC001, 1, 2, 62},
// {0, 0, 0, 0, 0},
// };
