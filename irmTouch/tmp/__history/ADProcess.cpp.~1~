...
void __fastcall ADProcess::LogADDataV4(FILE *fp, void *Head, uint8_t *Data, bool ClcCheckSum)
{
	softscan_ctrl_t ssc;
...
//	fseek(fp, 0, SEEK_END);
	...
	if((HeadV2->Type & 0xF0) == REPORT_MODE_VERSION4) {
		...
		for(int i = HeadV2->XAxis + HeadV2->YAxis; i<(HeadV2->XAxis + HeadV2->YAxis)*2; i+=1) {
			fprintf(fp, "%d ", HeadV2->Offsets[i]);
		}
	...
	if(ClcCheckSum) {
		...
		for(int i=0; i<HeadV2->XAxis*HeadV2->XLeds + HeadV2->YAxis*HeadV2->YLeds; i+=1) {
			CheckSum += Data[i];
		}
...
void __fastcall ADProcess::LogADData(FILE *fp, void *Head, uint8_t *Data, bool ClcCheckSum)
{
	DebugHeadV2_t *HeadV2 = (DebugHeadV2_t *)Head;
	uint32_t HeadLen = 11 + (HeadV2->XAxis + HeadV2->YAxis) * 2;
	uint8_t *Ptr = Data;
   ...
   //if(HeadV2->YLeds != 0) {
		...
		for(int i=0; i<Y_CORNER_AXIS; i+=1) {
			for(int j=0; j<MAX_Y_OFF; j+=1) {
				fprintf(fp, "%u ", Buf[i+XAXIS+YAXIS+X_CORNER_AXIS][j] & 0x0FF);
...
void __fastcall ADProcess::LogDebugADData(FILE *fp, DebugHead_t *Head, uint8_t *Data)
{
	DebugHeadV2_t *HeadV2 = (DebugHeadV2_t *)Head;
	...
	if((Head->Type & 0xF0) == REPORT_MODE_VERSION2)
	{
		...
		for(int i = Head->XAxis + Head->YAxis; i<(Head->XAxis + Head->YAxis)*2; i+=1)
		{
			fprintf(fp, "%d ", HeadV2->Offsets[i]);
		}
...
void __fastcall ADProcess::LogData(void *DebugHead, uint8_t *Data)
{
	...
	if(MainForm->ToolButtonRecord->Down != LastLogSta)
	{
		if(LastLogSta == false)	//open
		{
			...
			if((Logfp = fopen(AnsiString(ExtractFilePath(Application->ExeName) + String(ltime)+".txt").c_str(), "w")) == NULL)
			{
			...
			else if(LogType == REPORT_MODE_VERSION2)
			{
				...
				LogADData(Logfp, (DebugHead_t *)&HeadV2, MtBound.MaxVolt[0], true);

				//LogEnvData
				HeadV2.Type = 0x1 | REPORT_MODE_VERSION2;
				LogADData(Logfp, (DebugHead_t *)&HeadV2, MtBound.UsualVolt[0], true);

				//Logad
				HeadV2.Type = 0x0 | REPORT_MODE_VERSION2;
				LogADData(Logfp, (DebugHead_t *)&HeadV2, MtBound.AxisAd[0], true);
			}
				...
				LogADData_V5_Cur(Logfp, (DebugHead_t *)&HeadV2, MtBound.MaxVolt);

				//LogEnvData
				HeadV2.Type = 0x1 | REPORT_MODE_VERSION5;
				LogADData_V5_Cur(Logfp, (DebugHead_t *)&HeadV2, MtBound.UsualVolt);
	...
	if(MainForm->ToolButtonRecord->Down == true)
	{
		...
		{
			//Log Current Data
			if(LogType == REPORT_MODE_VERSION1 || LogType == REPORT_MODE_VERSION2){
...
{
	if(dh->XAxis != LastDebugHeadV2.XAxis || dh->YAxis != LastDebugHeadV2.YAxis)
	{
		goto __ParamUpdate_Update_V2;
	}

	if(dh->XLeds != LastDebugHeadV2.XLeds || dh->YLeds != LastDebugHeadV2.YLeds)
	{
		goto __ParamUpdate_Update_V2;
	}
...
//	}
	...
	if(Data[Len-1] != CheckSum) {
		//CheckSum Error;
		MainForm->StatusBar->SimpleText = L"校验和错误!" + String((uint32_t)Data[Len-1]) + L":" + String((uint32_t)CheckSum);
		if(fp) {
	...
	if(MainForm->MTime < MainForm->CTime) MainForm->MTime = MainForm->CTime;

	if(DebugDataVer == REPORT_MODE_VERSION1 || DebugDataVer == REPORT_MODE_VERSION2 ||
	   DebugDataVer == REPORT_MODE_VERSION4 || DebugDataVer == REPORT_MODE_VERSION5)
	{
		if((Data[0] & 0x0F) == 0  || (Data[0] & 0x0F) == 1 || (Data[0] & 0x0F) == 4)		//AD data
		{
		...
		{
			case 0:   			//ad value
				CornerSignals = Len -1 - HeadLen - dh->XAxis * dh->XLeds -  dh->YAxis * dh->YLeds;
			...
			case 1:     //usual voltage initial
				...
				if(DebugDataVer == REPORT_MODE_VERSION5) {
					...
					for(int i=0; i<4; i+=1) {
						memcpy(MtBound.UsualVolt[i + XAXIS+YAXIS], ActualData, MAX_X_OFF);
						ActualData += MAX_X_OFF;
					}

					for(int i=0; i<4; i+=1) {
						memcpy(MtBound.UsualVolt[i + XAXIS+YAXIS + 4], ActualData, MAX_Y_OFF);
			...
			case 4:
				//if(FrameInfo.Info.YLeds != 0) {
					...
					if(DebugDataVer == REPORT_MODE_VERSION5) {
						...
						for(int i=0; i<Y_CORNER_AXIS; i+=1) {
							memcpy(MtBound.MaxVolt[i + XAXIS+YAXIS + X_CORNER_AXIS], ActualData, MAX_Y_OFF);
							memcpy(MtBound.AxisAd[i + XAXIS+YAXIS + X_CORNER_AXIS], ActualData, MAX_Y_OFF);
							ActualData += MAX_Y_OFF;
						}
					}
//				} else {
...
void __fastcall ADProcess::ProcessAD(uint8_t ver, uint8_t EnvFlag, uint8_t *Data, uint32_t Len)
{
...
void __fastcall ADProcess::ProcessPoint(uint8_t EnvFlag, uint8_t *Data, uint32_t Len)
{
	point_t Point[MAX_POINTS];
