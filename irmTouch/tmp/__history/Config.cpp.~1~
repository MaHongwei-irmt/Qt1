...
bool __fastcall TFormConfig::Gen_DMATable_Rule_Check(uint32_t Flags)
{
	...
	if (XParam.Axis != YParam.Axis) {
		Msg.cat_printf(L"X,Y轴数不同(x=%d, y=%d)，目前不支持，请设定为相同值！", XParam.Axis, YParam.Axis);
	}
...
bool __fastcall TFormConfig::GenScanTable(void)
{
	...
	TabHead.Flags = (Flag & 0x0F) |
		(((uint8_t)CheckBoxDoubleFirst->Checked) << 4) |
		(((uint8_t)CheckBoxAntiGlare->Checked) << 5) |
	...
	for (int i = 0, Cuts = 0; i < XParam.Boards && Cuts < sizeof(TabHead.XCuts); i += 1)
	{
	...
	for (int i = 0, Cuts = 0; i < YParam.Boards && Cuts < sizeof(TabHead.YCuts); i += 1)
	{
	...
	if (CheckBoxSwitchEmit->Checked)         //switch Emit
	{
		...
		TabHead.EmitTabLen = TabHead.XLeds*(XAxis + SkipAxis) + TabHead.YLeds*(YAxis + SkipAxis);
		...
		if (CheckBoxDoubleFirst->Checked == true)
		{
			...
			TabHead.EmitTabLen += ((XAxis + SkipAxis) * TabHead.XBoards + (YAxis + SkipAxis) * TabHead.YBoards);
		}
	...
	else if (CheckBoxSwEmitRev->Checked)      //switch emit rev simultaneously
	{
		...
		TabHead.RevTabLen = TabHead.XLeds*(XAxis + SkipAxis) + TabHead.YLeds*(YAxis + SkipAxis);
		...
		XParam.Get_Emit_Data == Get_Emit_Data_PTV150)
	{
		for (int i = 0; i < YParam.Boards; i += 1) {
			...
			if ((YParam.BoradLeds[i] & 0x07) != 0) {
				TabHead.JumpLedsTableLen += (8 - (YParam.BoradLeds[i] & 0x07)) * 2;
		...
		for (int i = 0; i < XParam.Boards - 1; i += 1) {
			...
			if ((XParam.BoradLeds[i] & 0x07) != 0) {
				TabHead.JumpLedsTableLen += (8 - (XParam.BoradLeds[i] & 0x07)) * 2;
				TabHead.xJumps += (8 - (XParam.BoradLeds[i] & 0x07));
			}
		}

		if (WaitRxPowerOn) {
			TabHead.JumpLedsTableLen += (XParam.BoradLeds[XParam.Boards - 1] + 7) / 8 * 2;
		...
		if (CheckBoxSkipX->Checked == true)         //we need jump leds @ the end of x
		{
			...
			{
				TabHead.JumpLedsTableLen += (8 - (XParam.BoradLeds[XParam.Boards - 1] & 0x07)) * 2;
				TabHead.xJumps += (8 - (XParam.BoradLeds[XParam.Boards - 1] & 0x07));
			}

			if(XParam.Get_Emit_Data == Get_Emit_Data_PDV1334 || XParam.Get_Emit_Data == Get_Emit_Data_PDV1335) {
				if ((XParam.BoradLeds[XParam.Boards - 1] & 0x07) == 0) {
		...
		TabHead.EmitTabLen = (TabHead.XLeds + TabHead.xJumps)*(XAxis + SkipAxis) +
			(TabHead.YLeds + TabHead.yJumps)*(YAxis + SkipAxis);
	...
	//	}
		...
		TabHead.GPIOBLen = TabHead.GPIOCLen = TabHead.EmitTabLen * 4;	// for emit switch
		TabHead.EmitTabLen *= 2;
	} else if(XParam.Get_Emit_Data == Get_Emit_Data_PDV1331 || XParam.Get_Emit_Data == Get_Emit_Data_PDV1333 || \
			  XParam.Get_Emit_Data == Get_Emit_Data_PDV1334 || XParam.Get_Emit_Data == Get_Emit_Data_PDV1335) {
		if (WaitRxPowerOn) {
			TabHead.RevTabLen = (XAxis + SkipAxis) * 9 * 4;		//for hc4051 abc
		} else {
			TabHead.RevTabLen = (XAxis + SkipAxis) * 8 * 4;		//for hc4051 abc     axis * channels
		}
		...
		if(XParam.Axis>9 && YParam.Axis>9) {  //main axi only for dynamic scan
			TabHead.EmitTabLen = TabHead.XLeds + TabHead.YLeds;
			TabHead.RevTabLen = 0;
	...
	((uint32_t *)(Table + fp->ScanTabSize - 4))[0] = FormUpdate->CRC32((uint32_t *)Table, fp->ScanTabSize / 4 - 1);
...
void __fastcall TFormConfig::ButtonDownTabClick(TObject *Sender)
{
	...
	if (MainForm->HIDSendThread->SubmitTask(TYPE_APP, fp->ScanTabSize, Table, &Head))
	{
		DisableButtons();
...
//	TabSheet3->Enabled = false;
	...
	ComboBoxEmitXVer->Hint = "V4.6: W系列发射板 69灯 \n"
		"V5.4: T系列发射板/主板 \n"
...
void __fastcall TFormConfig::ComboBoxXAxisChange(TObject *Sender)
{
	...
	for(int i=0; i<MAX_AXIS; i+=1)
	{
		if (i < Offs)
		{
			...
			((TComboBox *)(XParamEdit->Items[i + MAX_AXIS * 3]))->Enabled = true;
			((TComboBox *)(XParamEdit->Items[i + MAX_AXIS * 4]))->Enabled = true;
		...
		else
		{
			...
			((TComboBox *)(XParamEdit->Items[i + MAX_AXIS * 3]))->Enabled = false;
			((TComboBox *)(XParamEdit->Items[i + MAX_AXIS * 4]))->Enabled = false;
...
void __fastcall TFormConfig::ComboBoxYAxisChange(TObject *Sender)
{
	...
	for(int i=0; i<MAX_AXIS; i+=1)
	{
		if (i < Axis)
		{
			...
			((TComboBox *)(YParamEdit->Items[i + MAX_AXIS * 3]))->Enabled = true;
			((TComboBox *)(YParamEdit->Items[i + MAX_AXIS * 4]))->Enabled = true;
		...
		else
		{
			...
			((TComboBox *)(YParamEdit->Items[i + MAX_AXIS * 3]))->Enabled = false;
			((TComboBox *)(YParamEdit->Items[i + MAX_AXIS * 4]))->Enabled = false;
	...
	for (int i = 0; i < Ap->Axis; i += 1) {
		if (i == 0) {
			...
			if (Ap->LedOff[i] < 0) {
				...
				Ap->MixCos[i * 2 - 1] = 1 / sqrt(1 + Ap->AuxTan[i] * Ap->AuxTan[i]);
...
void __fastcall TFormConfig::ClcAuxParam(TList *Edits, AXI_PARAM *Ap)
{
	...
	for (int i = 0; i < (Ap->Axis + 1) / 2; i += 1) {
		if (i == 0) {                //0
			...
			((TEdit *)(Edits->Items[i * 2 + MAX_AXIS * 5]))->Text = String(-1 * off);
			((TEdit *)(Edits->Items[i * 2 - 1 + MAX_AXIS * 5]))->Text = String(off);
	...
	for (int i = 0; i < Ap->Axis; i += 1)		//fix me!!!  轴偏移角度排序
	{
		...
		OffLed = _wtoi(((TEdit *)(Edits->Items[i + MAX_AXIS*5]))->Text.c_str());
		Offlen = OffLed * Ap->Dis;
		if (Offlen > 0) {
			Offlen += Ap->Dis / 2;
		}
...
void __fastcall TFormConfig::Button2Click(TObject *Sender)
{
	...
	XParam.Len = YParam.Dis * (YParam.TotalLeds) + YParam.Margin + YParam.MarginRightDown;
...
void __fastcall TFormConfig::Button3Click(TObject *Sender)
{
	...
	YParam.Len = XParam.Dis * (XParam.TotalLeds) + XParam.Margin + XParam.MarginRightDown;
...
void __fastcall TFormConfig::Button4Click(TObject *Sender)
{
	...
	for (int i = 0; i < XParam.Axis; i += 1)
	{
		...
		Gain1 = _wtoi( ((TComboBox *)XParamEdit->Items[i+MAX_AXIS*3])->Text.c_str());
		Gain2 = _wtoi( ((TComboBox *)XParamEdit->Items[i+MAX_AXIS*4])->Text.c_str());
	...
	for (int i = 0; i < YParam.Axis; i += 1)
	{
		...
		Gain1 = _wtoi( ((TComboBox *)YParamEdit->Items[i+MAX_AXIS*3])->Text.c_str());
		Gain2 = _wtoi( ((TComboBox *)YParamEdit->Items[i+MAX_AXIS*4])->Text.c_str());
...
void __fastcall TFormConfig::ComboBoxXBoardsChange(TObject *Sender)
{
	...
	if (Sender && (TComboBox *)Sender == ComboBoxXBoards)
	{
		for (i = 0; i < Cuts; i += 1)
		{
			...
			((TEdit *)XCutBoards->Items[i + Cuts])->Text = ComboBoxXBoardLeds->Text;
		}
		...
		for (i = 0; i < ap->Boards && i < Cuts; i += 1)
		{
			...
			((TComboBox *)XCutBoards->Items[i])->ItemIndex = ((TComboBox *)XCutBoards->Items[i])->Items->IndexOf(Msg);
		}
	...
	if (!CheckBox17->Checked) {
		...
		for (i = 0; i < Cuts; i += 1)
		{
			...
			{
				int BoardNo = _wtoi(((TComboBox *)XCutBoards->Items[i])->Text.c_str());

				if (BoardNo > 0 && BoardNo <= ap->Boards)
				{
					ap->BoradLeds[BoardNo-1] = _wtoi(((TEdit *)XCutBoards->Items[i+Cuts])->Text.c_str());
				}
			}
		}
	} else {
...
void __fastcall TFormConfig::ComboBoxYBoardsChange(TObject *Sender)
{
	...
	if(Sender && (TComboBox *)Sender == ComboBoxYBoards)
	{
		for (i = 0; i < Cuts; i += 1)
		{
			...
			((TEdit *)YCutBoards->Items[i + Cuts])->Text = ComboBoxYBoardLeds->Text;
		}
		...
		for (i = 0; i < ap->Boards && i < Cuts; i += 1)
		{
			...
			((TComboBox *)YCutBoards->Items[i])->ItemIndex = ((TComboBox *)YCutBoards->Items[i])->Items->IndexOf(Msg);
		}
	...
	if (!CheckBox19->Checked) {
		...
		for (i = 0; i < Cuts; i += 1)
		{
			...
			{
				int BoardNo = _wtoi(((TComboBox *)YCutBoards->Items[i])->Text.c_str());

				if (BoardNo > 0 && BoardNo <= ap->Boards)
				{
					ap->BoradLeds[BoardNo-1] = _wtoi(((TEdit *)YCutBoards->Items[i+Cuts])->Text.c_str());
				}
			}
		}
	} else {
...
void __fastcall TFormConfig::TimerDownTabTimer(TObject *Sender)
{
	...
	else
	{
		...
		{
			Percent = MainForm->HIDSendThread->SendLen * 100 / MainForm->HIDSendThread->Len;
		}
...
void __fastcall TFormConfig::ButtonDownParamClick(TObject *Sender)
{
	...
	if (sizeof(ParamBuf) < sizeof(Param) + 4 || sizeof(ParamBuf) < fp->ParamSize)            //4bytes for crc32
	{
	...
	((uint32_t *)(ParamBuf + fp->ParamSize - 4))[0] = FormUpdate->CRC32((uint32_t *)ParamBuf, fp->ParamSize / 4 - 1);
	...
	if (MainForm->HIDSendThread->SubmitTask(TYPE_APP,
		fp->ParamSize,
		ParamBuf,
		&Head))
	{
...
void __fastcall TFormConfig::Button8Click(TObject *Sender)
{
	...
	if (MessageBox(NULL, L"确定删除该配置?\n扫描表和参数配置将同时被删除!", L"删除配置", MB_OKCANCEL | MB_ICONWARNING) == IDCANCEL)
	{
		return;
	}
...
void __fastcall TFormConfig::SaveScanTableConfig(TIniFile *ini, String Section)
{
	...
	for (int i = 0; i < Axis; i += 1)
	{
		...
		ini->WriteString(Section, Msg, ((TComboBox *)XParamEdit->Items[i + MAX_AXIS * 3])->Text);

		Msg.sprintf(L"X_Gain2_Off%d", i + 1);
		ini->WriteString(Section, Msg, ((TComboBox *)XParamEdit->Items[i + MAX_AXIS * 4])->Text);

		Msg.sprintf(L"X_Corner_LedOff%d", i + 1);
		ini->WriteString(Section, Msg, ((TComboBox *)XParamEdit->Items[i + MAX_AXIS * 5])->Text);
	}
	...
	for (int i = 0; i < Boards && i < Cuts; i += 1)
	{
		...
		ini->WriteString(Section, "XCutBoard" + String(i + 1), ((TComboBox*)XCutBoards->Items[i])->Text);
		ini->WriteString(Section, Keys, ((TEdit*)XCutBoards->Items[i + Cuts])->Text);
	}
	...
	for (int i = 0; i < Axis; i += 1)
	{
		...
		ini->WriteString(Section, Msg, ((TComboBox *)YParamEdit->Items[i + MAX_AXIS * 3])->Text);

		Msg.sprintf(L"Y_Gain2_Off%d", i + 1);
		ini->WriteString(Section, Msg, ((TComboBox *)YParamEdit->Items[i + MAX_AXIS * 4])->Text);

		Msg.sprintf(L"Y_Corner_LedOff%d", i + 1);
		ini->WriteString(Section, Msg, ((TComboBox *)YParamEdit->Items[i + MAX_AXIS * 5])->Text);
	}
	...
	for (int i = 0; i < Boards && i < Cuts; i += 1)
	{
		...
		ini->WriteString(Section, "YCutBoard" + String(i + 1), ((TComboBox*)YCutBoards->Items[i])->Text);
		ini->WriteString(Section, Keys, ((TEdit*)YCutBoards->Items[i + Cuts])->Text);
	}
	...
	ini->WriteBool(Section, "CPLDMagicConfig", CheckBoxCPLDMagicConfig->Checked);
...
void __fastcall TFormConfig::SaveParamConfig(TIniFile *ini, String Section)
{
	...
	ini->WriteBool(Section, "FeatureInterplation", CheckBoxFeatureInterplation->Checked);
	ini->WriteBool(Section, "FeatureMac", CheckBoxFeatureMac->Checked);
	ini->WriteBool(Section, "FeatureReverseX", CheckBoxFeatureReverseX->Checked);
	ini->WriteBool(Section, "FeatureReverseY", CheckBoxFeatureReverseY->Checked);
	ini->WriteBool(Section, "FeatureExchangeXY", CheckBoxFeatureExchangeXY->Checked);
	ini->WriteBool(Section, "FeatureUSARTOn", CheckBoxFeatureUSARTOn->Checked);
	ini->WriteBool(Section, "FeatureSmallPointOn", CheckBoxFeatureSmallPointOn->Checked);
	ini->WriteBool(Section, "FeatureFourceMouse", CheckBoxFeatureFourceMouse->Checked);
	...
	ini->WriteBool(Section, "FeatureBoardPoint", CheckBoxFeatureBoardPoint->Checked);
	ini->WriteBool(Section, "FeatureGainAdj", CheckBoxFeatureGainAdj->Checked);
	ini->WriteBool(Section, "FeatureEdgeAdsorption", CheckBoxFeatureEdgeAdsorption->Checked);
	ini->WriteBool(Section, "FeatureGPIOGesture", CheckBoxFeatureGPIOGesture->Checked);
	ini->WriteBool(Section, "FeatureDisableRightClick", CheckBoxFeatureDisableRightClick->Checked);
	ini->WriteBool(Section, "CheckBoxD2CurHigh", CheckBoxD2CurHigh->Checked);
	ini->WriteBool(Section, "FeatureAutoGainSub", CheckBoxFeatureAutoGainSub->Checked);
	ini->WriteBool(Section, "FeatureAutoGainUniform", CheckBoxFeatureAutoGainUniform->Checked);
...
void __fastcall TFormConfig::ComboBoxStdScreenChange(TObject *Sender)
{
	...
	{
		Msg.sprintf(L"X_ExpectedDeg_Off%d", i + 1);
		((TEdit *)XParamEdit->Items[i])->Text = ini->ReadString(Section, Msg, "0");

		Msg.sprintf(L"X_Gain1_Off%d", i + 1);
		((TComboBox *)XParamEdit->Items[i + MAX_AXIS * 3])->Text = ini->ReadString(Section, Msg, "2");

		Msg.sprintf(L"X_Gain2_Off%d", i + 1);
		((TComboBox *)XParamEdit->Items[i + MAX_AXIS * 4])->Text = ini->ReadString(Section, Msg, "2");

		Msg.sprintf(L"X_Corner_LedOff%d", i + 1);
		((TEdit *)XParamEdit->Items[i + MAX_AXIS * 5])->Text = ini->ReadString(Section, Msg, "0");
	}
	...
	EditXMarginRight->Text = ini->ReadString(Section, "XMarginRight", EditXMargin->Text);
	...
	{
		Keys.printf(L"XTail%dLeds", i + 1);
		Msg = ini->ReadString(Section, "XCutBoard" + String(i + 1), String(Boards - i));
		((TComboBox*)XCutBoards->Items[i])->ItemIndex = ((TComboBox*)XCutBoards->Items[i])->Items->IndexOf(Msg);
		((TEdit*)XCutBoards->Items[i + Cuts])->Text = ini->ReadString(Section, Keys, ComboBoxXBoardLeds->Text);
	}
	...
	ComboBoxEmitXVer->ItemIndex = ComboBoxEmitXVer->Items->IndexOf(ComboBoxEmitXVer->Text);
	ComboBoxRevXVer->ItemIndex = ComboBoxRevXVer->Items->IndexOf(ComboBoxRevXVer->Text);
	...
	for (int i = 0; i < Axis; i += 1) {
		Msg.sprintf(L"Y_ExpectedDeg_Off%d", i + 1);
		((TEdit *)YParamEdit->Items[i])->Text = ini->ReadString(Section, Msg, "0");

		Msg.sprintf(L"Y_Gain1_Off%d", i + 1);
		((TComboBox *)YParamEdit->Items[i + MAX_AXIS * 3])->Text = ini->ReadString(Section, Msg, "2");

		Msg.sprintf(L"Y_Gain2_Off%d", i + 1);
		((TComboBox *)YParamEdit->Items[i + MAX_AXIS * 4])->Text = ini->ReadString(Section, Msg, "2");

		Msg.sprintf(L"Y_Corner_LedOff%d", i + 1);
		((TEdit *)YParamEdit->Items[i + MAX_AXIS * 5])->Text = ini->ReadString(Section, Msg, "0");
	}
	...
	EditYMarginDown->Text = ini->ReadString(Section, "YMarginDown", EditYMargin->Text);
	...
	for (int i = 0; i < Boards && i < Cuts; i += 1)
	{
		...
		Msg = ini->ReadString(Section, "YCutBoard" + String(i + 1), String(Boards - i));
		((TComboBox*)YCutBoards->Items[i])->ItemIndex = ((TComboBox*)YCutBoards->Items[i])->Items->IndexOf(Msg);
		((TEdit*)YCutBoards->Items[i + Cuts])->Text = ini->ReadString(Section, Keys, ComboBoxYBoardLeds->Text);
	}
	...
	ComboBoxEmitYVer->ItemIndex = ComboBoxEmitYVer->Items->IndexOf(ComboBoxEmitYVer->Text);
	ComboBoxRevYVer->ItemIndex = ComboBoxRevYVer->Items->IndexOf(ComboBoxRevYVer->Text);
	...
	CheckBoxSkipFirstAxis->Checked = ini->ReadBool(Section, "SkipFirstAxis", true);
	...
	CheckBoxShutDownEmit->Checked = ini->ReadBool(Section, "ShutdownEmit", false);
	CheckBox8BitSPI->Checked = ini->ReadBool(Section, "SPI8Bit", false);
	CheckBoxGPIOGain->Checked = ini->ReadBool(Section, "GPIOGain", false);
	CheckBoxReverseXScan->Checked = ini->ReadBool(Section, "ReverseXScan", false);
	CheckBoxReverseYScan->Checked = ini->ReadBool(Section, "ReverseYScan", false);
	...
	CheckBoxCPLDMagicConfig->Checked = ini->ReadBool(Section, "CPLDMagicConfig", false);
	...
	CheckBoxWaitRxPowerOn->Checked = ini->ReadBool(Section, "WaitRxPowerOn", false);
...
void __fastcall TFormConfig::DispSta(void)
{
	...
	Msg.sprintf(L"%d.%d", (BootParam.BootLoaderVersion & 0x7F) / 10, (BootParam.BootLoaderVersion & 0x7F) % 10);
	...
	{
		Msg.sprintf(L"%d.%d", BootParam.HardwareVer / 100, BootParam.HardwareVer % 100);
	}
	else
	{
		Msg.sprintf(L"N/A");
	...
	if (BootParam.FlashSize * 32 == 64)
		...
		if(BootParam.BootLoaderVersion >= 80 && BootParam.BootLoaderVersion < 90) {
...
void __fastcall TFormConfig::Button10Click(TObject *Sender)
{
	...
	Msg.sprintf(L"%d-%d-%d", tTime.tm_year + 1900, tTime.tm_mon + 1, tTime.tm_mday);
	EditDate->Text = Msg;
	...
	//生成 CPUID
	if ((CPUID[0] == 0x02 && CPUID[1] == 0x03 && CPUID[2] == 0x04 && CPUID[3] == 0x05 && CPUID[4] == 0x06) ||
		(CPUID[0] == 0xFF && CPUID[1] == 0xFF && CPUID[2] == 0xFF && CPUID[3] == 0xFF && CPUID[4] == 0xFF)) {
		String Msg = "";
		for (int i = 0; i < 12; i += 1) {
			CPUID[i] = rand();
...
bool __fastcall TFormConfig::CheckParam(void)
{
	...
	{
		if (atoi(Ptr + 1) != GetMaxNum(BootParam.Flags >> 4) && Param.tracepoints != atoi(Ptr + 1))
		{
	...
	else if (BootParam.FlashSize * 32 == 128)
	{
		if ((BootParam.BootLoaderVersion & 0x7F) >= 60)
		{
			if (BootParam.HardwareVer == 540 ||
				BootParam.HardwareVer == 650 ||
				BootParam.HardwareVer == 700 ||
				BootParam.HardwareVer == 1000 ||
...
bool __fastcall TFormConfig::WriteDataBase(void)
{
	...
	if (CheckParam() == false)	return false;
	...
	{
		if (MessageBox(NULL, L"CPUID已经存在，需要更新吗?", L"确认更新", MB_OKCANCEL | MB_ICONWARNING) == IDCANCEL)
		{
			fclose(fp);
			return false;
		}
	...
	memcpy(Ptr, CPUID, sizeof(CPUID));   Ptr += sizeof(CPUID);
	memcpy(Ptr, &ShipTime, sizeof(ShipTime)); Ptr += sizeof(ShipTime);
...
void __fastcall TFormConfig::ShowAutoGainResult(uint8_t *Buf, uint16_t Len)
{
	...
	for (int i = 1; i < StringGrid1->ColCount && i < Boards + 1; i += 1)
	{
		...
		StringGrid1->Cells[i][5] = String((uint32_t)(Buf[i + Boards*2] & 0x0F));
		StringGrid1->Cells[i][6] = String((uint32_t)(Buf[i + Boards*3] & 0x0F));
		StringGrid1->Cells[i][7] = String((uint32_t)(Buf[i + Boards*4] & 0x0F));
		StringGrid1->Cells[i][8] = String((uint32_t)(Buf[i + Boards*5] & 0x0F));
		StringGrid1->Cells[i][9] = String((uint32_t)(Buf[i + Boards*6] & 0x0F));
		StringGrid1->Cells[i][10] = String((uint32_t)(Buf[i + Boards*7] & 0x0F));
		StringGrid1->Cells[i][11] = String((uint32_t)(Buf[i + Boards*8] & 0x0F));
		StringGrid1->Cells[i][12] = String((uint32_t)(Buf[i + Boards*9] & 0x0F));
		StringGrid1->Cells[i][13] = String((uint32_t)(Buf[i + Boards*10] & 0x0F));
		StringGrid1->Cells[i][14] = String((uint32_t)(Buf[i + Boards*11] & 0x0F));
		StringGrid1->Cells[i][15] = String((uint32_t)(Buf[i + Boards*12] & 0x0F));
		StringGrid1->Cells[i][16] = String((uint32_t)(Buf[i + Boards*13] & 0x0F));
	}
}
...
const char *EmitVerStringMap[] = {
	...
	"MainV6.9 EmitV6.8",
	"X-V14.1",
	"MainV6.9-3N EmitV6.8",
	"MainV6.9-3N EmitV6.9",
	"V5.6",
	"X-21.5-V14.2",
	"E19-V14.3",
	"E15-V14.4",
...
void __fastcall TFormConfig::ShowScanParam(uint8_t *Buf, uint16_t Len)
{
	...
	if (Len >= (uint16_t)(&(((SCAN_TABLE_HEAD *)0)->Feature)) + 4)
	{
		...
		for (i = 0, Cuts = 0; Cuts < XCutBoards->Count / 2 && i < sizeof(sp->XCuts); i += 2)
		{
			if (sp->XCuts[i + 1] != 0)
			{
				Msg.sprintf(L"%d", sp->XCuts[i] + 1);
				((TComboBox *)(XCutBoards->Items[Cuts]))->Text = Msg;
				((TComboBox *)(XCutBoards->Items[Cuts]))->ItemIndex = ((TComboBox *)(XCutBoards->Items[Cuts]))->Items->IndexOf(Msg);

				Msg.sprintf(L"%d", sp->XCuts[i + 1]);
				((TEdit *)(XCutBoards->Items[Cuts + XCutBoards->Count / 2]))->Text = Msg;
		...
		for (; Cuts < XCutBoards->Count / 2; Cuts += 1)
		{
			...
			((TComboBox *)(XCutBoards->Items[Cuts]))->ItemIndex = ((TComboBox *)(XCutBoards->Items[Cuts]))->Items->IndexOf("0");
			((TEdit *)(XCutBoards->Items[Cuts + XCutBoards->Count / 2]))->Text = "0";
		}
		...
		if (sp->HIDToolsVer < 3004 || sp->XCutUnUsed != 0xAA) {
			for (i = 0; i < XCutBoards->Count / 2; i += 1)
			{
				...
				{
					((TComboBox *)(XCutBoards->Items[i]))->Enabled = true;
					((TEdit *)(XCutBoards->Items[i + XCutBoards->Count / 2]))->Enabled = true;
				}
				else
				{
					((TComboBox *)(XCutBoards->Items[i]))->Enabled = false;
					((TEdit *)(XCutBoards->Items[i + XCutBoards->Count / 2]))->Enabled = false;
				}
			}
		}
		...
		if (sp->HIDToolsVer >= 2082) {
			...
			if (sp->XEmitVer < EmitVerStringMapSize) {
				ComboBoxEmitXVer->ItemIndex = ComboBoxEmitXVer->Items->IndexOf(String(EmitVerStringMap[sp->XEmitVer]));
			} else {
				ComboBoxEmitXVer->ItemIndex = 0;
			}

			if (sp->XRevVer < RevVerStringMapSize) {
				ComboBoxRevXVer->ItemIndex = ComboBoxRevXVer->Items->IndexOf(String(RevVerStringMap[sp->XRevVer]));
			} else {
		...
		for (i = 0, Cuts = 0; Cuts < YCutBoards->Count / 2 && i < sizeof(sp->YCuts); i += 2)
		{
			if (sp->YCuts[i + 1] != 0)
			{
				Msg.sprintf(L"%d", sp->YCuts[i] + 1);
				((TComboBox *)(YCutBoards->Items[Cuts]))->Text = Msg;
				((TComboBox *)(YCutBoards->Items[Cuts]))->ItemIndex = ((TComboBox *)(YCutBoards->Items[Cuts]))->Items->IndexOf(Msg);

				Msg.sprintf(L"%d", sp->YCuts[i + 1]);
				((TEdit *)(YCutBoards->Items[Cuts + YCutBoards->Count / 2]))->Text = Msg;
		...
		for (; Cuts < YCutBoards->Count / 2; Cuts += 1)
		{
			...
			((TComboBox *)(YCutBoards->Items[Cuts]))->ItemIndex = ((TComboBox *)(YCutBoards->Items[Cuts]))->Items->IndexOf("0");
			((TEdit *)(YCutBoards->Items[Cuts + YCutBoards->Count / 2]))->Text = "0";
		}
		...
		if (sp->HIDToolsVer < 3004 || sp->YCutUnUsed != 0xAA) {
			for (i = 0; i < YCutBoards->Count / 2; i += 1)
			{
				...
				{
					((TComboBox *)(YCutBoards->Items[i]))->Enabled = true;
					((TEdit *)(YCutBoards->Items[i + YCutBoards->Count / 2]))->Enabled = true;
				}
				else
				{
					((TComboBox *)(YCutBoards->Items[i]))->Enabled = false;
					((TEdit *)(YCutBoards->Items[i + YCutBoards->Count / 2]))->Enabled = false;
				}
			}
		}
		...
		if (sp->HIDToolsVer >= 2082) {
			...
			if (sp->YEmitVer < EmitVerYStringMapSize) {
				ComboBoxEmitYVer->ItemIndex = ComboBoxEmitYVer->Items->IndexOf(String(EmitVerYStringMap[sp->YEmitVer]));
			} else {
				ComboBoxEmitYVer->ItemIndex = 0;
			}

			if (sp->YRevVer < RevVerStringMapSize) {
				ComboBoxRevYVer->ItemIndex = ComboBoxRevYVer->Items->IndexOf(String(RevVerStringMap[sp->XRevVer]));
			} else {
		...
		if (CheckBoxAntiGlare->Checked && sp->Maigc1 == 0x13579ABC && sp->HIDToolsVer > 2071) {
...
void __fastcall TFormConfig::ShowParam(uint8_t *Buf, uint16_t Len)
{
	...
	if (FeatureCheckBox == NULL) return;
	...
	{
		if (i > 45) {
			StringGrid1->Cells[i][1] = String((uint32_t)((Param.Gain1[i - 46] >> 4) & 0x07));
			StringGrid1->Cells[i][2] = String((uint32_t)((Param.Gain2[i - 46] >> 4) & 0x03));

			if (this->BootParam.HardwareVer == 540) {
				StringGrid1->Cells[i][3] = String((uint32_t)((Param.EmitPow[i - 46] >> 4) & 0x07));
			} else {
				StringGrid1->Cells[i][3] = "0";
			}
		} else if(i>30){
			StringGrid1->Cells[i][1] = String((uint32_t)(Param.Gain1[i - 31] & 0x07));
			StringGrid1->Cells[i][2] = String((uint32_t)(Param.Gain2[i - 31] & 0x03));
			if (this->BootParam.HardwareVer == 540) {
				StringGrid1->Cells[i][3] = String((uint32_t)(Param.EmitPow[i - 31] & 0x07));
			} else {
				StringGrid1->Cells[i][3] = String((uint32_t)(Param.EmitPow[i - 31]));
			}
		} else if(i >15) {
			StringGrid1->Cells[i][1] = String((uint32_t)((Param.Gain1[i - 16] >> 4) & 0x07));
			StringGrid1->Cells[i][2] = String((uint32_t)((Param.Gain2[i - 16] >> 4) & 0x03));

			if (this->BootParam.HardwareVer == 540) {
				StringGrid1->Cells[i][3] = String((uint32_t)((Param.EmitPow[i - 16] >> 4) & 0x07));
			} else {
				StringGrid1->Cells[i][3] = "0";
			}
		} else {
			...
			if (this->BootParam.HardwareVer == 540) {
				...
				StringGrid1->Cells[i][3] = String((uint32_t)(Param.EmitPow[i - 1]));
			}
		}
	}
...
void __fastcall TFormConfig::Button13Click(TObject *Sender)
{
	...
	if (EditCPUID->Text.Length() == 24)
	{
		...
		{
			CPUID[i / 2] = hexfile.HexToInt(AnsiString(EditCPUID->Text).c_str() + i, 2);
		}
	...
	else if (EditSerialNum->Text.Length() == 14 || EditSerialNum->Text.Length() == 12)
	{
		String Msg = "";
...
void __fastcall TFormConfig::ButtonWriteSerialNumClick(TObject *Sender)       //set screen
{
...
//	LengthProInfo = EditDispProInfo->Text.Length();
	...
	((uint32_t *)(SerialBuf + fp->SerialSize - 4))[0] = FormUpdate->CRC32((uint32_t *)SerialBuf, fp->SerialSize / 4 - 1);
	...
	if (MainForm->HIDSendThread->SubmitTask(TYPE_APP, fp->SerialSize, SerialBuf, &Head))
	{
		DisableButtons();
		TimerDownTab->Enabled = true;
	}
...
void __fastcall TFormConfig::ComboBoxStdParamChange(TObject *Sender)
{
	...
	EditLowFrameTimeout->Text = ini->ReadString(Section, "LowFrameTimeout", "10");
	...
	EmitPow = ini->ReadString(Section, "EmitPow", "7777777777777777777777777777");
	...
	CheckBoxFeatureSmooth->Checked = ini->ReadBool(Section, "FeatureSmooth", true);
	CheckBoxFeatureInterplation->Checked = ini->ReadBool(Section, "FeatureInterplation", true);
	CheckBoxFeatureMac->Checked = ini->ReadBool(Section, "FeatureMac", false);
	CheckBoxFeatureReverseX->Checked = ini->ReadBool(Section, "FeatureReverseX", false);
	CheckBoxFeatureReverseY->Checked = ini->ReadBool(Section, "FeatureReverseY", true);
	CheckBoxFeatureExchangeXY->Checked = ini->ReadBool(Section, "FeatureExchangeXY", false);
	CheckBoxFeatureUSARTOn->Checked = ini->ReadBool(Section, "FeatureUSARTOn", false);
	CheckBoxFeatureSmallPointOn->Checked = ini->ReadBool(Section, "FeatureSmallPointOn", false);
	CheckBoxFeatureFourceMouse->Checked = ini->ReadBool(Section, "FeatureFourceMouse", false);
	...
	CheckBoxFeatureBoardPoint->Checked = ini->ReadBool(Section, "FeatureBoardPoint", false);
	CheckBoxFeatureGainAdj->Checked = ini->ReadBool(Section, "FeatureGainAdj", false);
	CheckBoxFeatureEdgeAdsorption->Checked = ini->ReadBool(Section, "FeatureEdgeAdsorption", false);
	CheckBoxFeatureGPIOGesture->Checked = ini->ReadBool(Section, "FeatureGPIOGesture", false);
	CheckBoxFeatureDisableRightClick->Checked = ini->ReadBool(Section, "FeatureDisableRightClick", false);
	CheckBoxD2CurHigh->Checked = ini->ReadBool(Section, "CheckBoxD2CurHigh", false);
	CheckBoxFeatureAutoGainSub->Checked = ini->ReadBool(Section, "FeatureAutoGainSub", false);
	CheckBoxFeatureAutoGainUniform->Checked = ini->ReadBool(Section, "FeatureAutoGainUniform", false);
...
void __fastcall TFormConfig::ButtonSecretInitialClick(TObject *Sender)
{
	...
	((uint32_t *)(SecretBuf + fp->SecretSize - 4))[0] = FormUpdate->CRC32((uint32_t *)SecretBuf, fp->SecretSize / 4 - 1);
	...
	if (MainForm->HIDSendThread->SubmitTask(TYPE_APP, fp->SecretSize, SecretBuf, &Head))
	{
		DisableButtons();
		TimerDownTab->Enabled = true;
	}
...
void __fastcall TFormConfig::Button19Click(TObject *Sender)
{
	...
	if ((TButton *)Sender == Button19)
		((uint32_t *)(ParamBuf + fp->CalibrateSize - 4))[0] = FormUpdate->CRC32((uint32_t *)ParamBuf, fp->CalibrateSize / 4 - 1);

	Head.MemType = TYPE_CALIB;
	Head.Mode = MODE_PROGRAM;
	Head.StartAdd = fp->CalibrateStartAdd;

	if (MainForm->HIDSendThread->SubmitTask(TYPE_APP, fp->CalibrateSize, ParamBuf, &Head))
	{
		DisableButtons();
		TimerDownTab->Enabled = true;
	}
...
bool Calibration_ClcParam(Calibration_t *cp, Calib_Info_t *ci)
{
	...
	else
	{
		...
		cp->Calib[0].k[0] = ((c[0] - c[2])*(b[1] - b[2]) - (c[1] - c[2])*(b[0] - b[2])) / k;
		cp->Calib[0].k[1] = ((c[1] - c[2])*(a[0] - a[2]) - (c[0] - c[2])*(a[1] - a[2])) / k;
		cp->Calib[0].k[2] = (b[0] * (a[2] * c[1] - a[1] * c[2]) + b[1] * (a[0] * c[2] - a[2] * c[0]) + b[2] * (a[1] * c[0] - a[0] * c[1])) / k;

		cp->Calib[0].k[3] = ((d[0] - d[2])*(b[1] - b[2]) - (d[1] - d[2])*(b[0] - b[2])) / k;
		cp->Calib[0].k[4] = ((d[1] - d[2])*(a[0] - a[2]) - (d[0] - d[2])*(a[1] - a[2])) / k;
		cp->Calib[0].k[5] = (b[0] * (a[2] * d[1] - a[1] * d[2]) + b[1] * (a[0] * d[2] - a[2] * d[0]) + b[2] * (a[1] * d[0] - a[0] * d[1])) / k;
...
void __fastcall TFormConfig::Button18Click(TObject *Sender)
{
	...
	{
		if (Calibration_ClcParam(&CalibParam, &FormCalibrate->CalibInfo))		//计算 校准系数
		{
			//临时写入触摸屏
#if CALIB_DEBUG == 1
			MainForm->HIDSendThread->SubmitCmd(APP_CMD_SET_CALIB_COFF, (uint8_t *)CalibParam.Calib[0].k, sizeof(CalibParam.Calib[0].k));
#else
...
void __fastcall TFormConfig::Button16Click(TObject *Sender)
{
	...
	if (sizeof(ParamBuf) < sizeof(RunTimeParam_t) + 4 || sizeof(ParamBuf) < fp->ParamSize)            //4bytes for crc32
	{
		ShowMessage(L"参数太多，需要修改程序！");
		return;
	}
...
void __fastcall TFormConfig::Button24Click(TObject *Sender)
{
	...
	if (OpenDialogConfig->Execute())
	{
		...
		TIniFile *iniImp = new TIniFile(ExtractFilePath(Application->ExeName) + "screen.ini");
		...
		if (ini && iniImp)
		{
			...
			for (int i = 0; i < FormConfigList->SelCount; i += 1)
			{
				...
				if (Ptr)
				{
					SectionKey = Sels.SubString(1, Ptr - AnsiString(Sels).c_str());
					...
					if (Value->Count != 0)
					{
						...
						if (MessageBox(NULL, Msg.t_str(), L"配置覆盖", MB_OKCANCEL | MB_ICONWARNING) == IDCANCEL)
						{
							continue;
						}
					}
					...
					for (int j = 0; j < Value->Count; j += 1)
					{
						...
						Val = Key.SubString(Ptr - AnsiString(Key).c_str() + 2, Key.Length() - (Ptr - AnsiString(Key).c_str() + 1));
						Key.Delete(Ptr - AnsiString(Key).c_str() + 1, Key.Length() - (Ptr - AnsiString(Key).c_str()));
			...
			ComboBoxStdScreen->ItemIndex = ComboBoxStdScreen->Items->IndexOf(Msg);
...
void __fastcall TFormConfig::Button25Click(TObject *Sender)
{
	...
	if (SaveDialogConfig->Execute())
	{
		...
		if (FileName.Length() < 4 || FileName.SubString(FileName.Length() - 3, 4) != ".txt")
		{
			FileNamePath += ".txt";
		}
		else
		...
		{
			if (MessageBox(NULL, L"文件已经存在，需要覆盖吗?", L"确认覆盖", MB_OKCANCEL | MB_ICONWARNING) == IDCANCEL)
			{
				return;
			}
...
void __fastcall TFormConfig::Button26Click(TObject *Sender)
{
	...
	if (SaveDialogConfig->Execute())
	{
		...
		if (FileName.Length() < 3 || FileName.SubString(FileName.Length() - 1, 2) != ".h")
		{
			FileNamePath += ".h";
		}
		else
...
void __fastcall TFormConfig::ComboBoxYDivMethodChange(TObject *Sender)
{
	...
	case 1: uniform = true; MidAdjAxis = 3;   break;
	case 2: uniform = true; MidAdjAxis = 5;   break;
	case 3: uniform = false; MidAdjAxis = 3;  break;
	case 4: uniform = false; MidAdjAxis = 5;  break;
	default: uniform = true; MidAdjAxis = 1;  break;
	}
	...
	if (Axis == 1) {
		...
		if (MidAdjAxis >= 3) {
			YParam.RequestDegOff[1] = atan(-1 * YParam.Dis / YParam.Len) / PI * 180;
			YParam.RequestDegOff[2] = atan(1 * YParam.Dis / YParam.Len) / PI * 180;
		}

		if (MidAdjAxis >= 5) {
			YParam.RequestDegOff[3] = atan(-2 * YParam.Dis / YParam.Len) / PI * 180;
			YParam.RequestDegOff[4] = atan(2 * YParam.Dis / YParam.Len) / PI * 180;
		}
		...
		if (Axis > MidAdjAxis) {
			if (uniform) {
				...
				for (int i = MidAdjAxis; i < Axis; i += 4) {
					...
					len = YParam.Len * tan(YParam.RequestDegOff[i + 3] / 180.0*PI);

					YParam.RequestDegOff[i + 1] = atan((len - YParam.Dis) / YParam.Len) / PI * 180;
					YParam.RequestDegOff[i + 0] = -1 * YParam.RequestDegOff[i + 1];
				}
			}
		}
	}

	YParam.MaxOff = (YParam.Len * tan(MaxYOff / 180.0*PI) + YParam.Dis / 2) / YParam.Dis;
...
void __fastcall TFormConfig::ComboBoxXDivMethodChange(TObject *Sender)
{
	...
	if (Axis == 1) {
		...
		if (MidAdjAxis >= 3) {
			XParam.RequestDegOff[1] = atan(-1 * XParam.Dis / XParam.Len) / PI * 180;
			XParam.RequestDegOff[2] = atan(1 * XParam.Dis / XParam.Len) / PI * 180;
		}

		if (MidAdjAxis >= 5) {
			XParam.RequestDegOff[3] = atan(-2 * XParam.Dis / XParam.Len) / PI * 180;
			XParam.RequestDegOff[4] = atan(2 * XParam.Dis / XParam.Len) / PI * 180;
		}
		...
		if (Axis > MidAdjAxis) {
			if (uniform) {
				...
				for (int i = MidAdjAxis; i < Axis; i += 4) {
					...
					len = XParam.Len * tan(XParam.RequestDegOff[i + 3] / 180.0*PI);

					XParam.RequestDegOff[i + 1] = atan((len - XParam.Dis) / XParam.Len) / PI * 180;
					XParam.RequestDegOff[i + 0] = -1 * XParam.RequestDegOff[i + 1];
				}
			}
		}
	}

	XParam.MaxOff = (XParam.Len * tan(MaxXOff / 180.0*PI) + XParam.Dis / 2) / XParam.Dis;
...
void __fastcall TFormConfig::CheckBox17Click(TObject *Sender)
{
	...
	if (ComboBoxXBoards->Enabled) {
		...
		for (int i = 0; i < XCutBoards->Count / 2; i += 1) {
			...
			((TEdit *)XCutBoards->Items[i + XCutBoards->Count / 2])->Enabled = !CheckBox17->Checked;
		}
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TFormConfig::CheckBox19Click(TObject *Sender)
{
	...
	if (ComboBoxYBoards->Enabled) {
		...
		for (int i = 0; i < YCutBoards->Count / 2; i += 1) {
			...
			((TEdit *)YCutBoards->Items[i + XCutBoards->Count / 2])->Enabled = !CheckBox19->Checked;
		}
	}
}
...
void __fastcall TFormConfig::EditXBoardLedChange(TObject *Sender)
{
	...
	if (CheckBox17->Checked) {
		XParam.Boards = ToBoardLeds(AnsiString(EditXBoardLed->Text).c_str(), XParam.BoradLeds);
...
void __fastcall TFormConfig::EditYBoardLedChange(TObject *Sender)
{
	...
	if (CheckBox19->Checked) {
		YParam.Boards = ToBoardLeds(AnsiString(EditYBoardLed->Text).c_str(), YParam.BoradLeds);
...
{
	if (this->BootParam.HardwareVer == 540 || this->BootParam.HardwareVer == 460) {
		StringGrid1->Cells[0][3] = "发射功率";
	} else {
