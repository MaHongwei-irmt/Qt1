...
//---------------------------------------------------------------------------
__fastcall TFormCalibrate::TFormCalibrate(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TFormCalibrate::ResizeBitMap(void)
{
	...
	if(SrcBitmap->Width != ImageCalibrate->Width || SrcBitmap->Height != ImageCalibrate->Height)
	{
...
void __fastcall TFormCalibrate::DrawBack(void)
{
	...
	SrcBitmap->Canvas->TextOutW(xPxis/2 - 140, 20, L"Press 'ESC' key or dual-touch more than 3 seconds to Exit!");
...
void __fastcall TFormCalibrate::UpdatePoint(int No, int Percent)
{
	...
	if(No < 0 || No >= ci->TotalPoints)	return;
...
//		x = xStartPxis + x*xStepPxis;
	...
	if(Percent != 0)
	{
		ex = x;
		ey = y-30;
		sx = x+ 30 * cos(-2*3.1415926*float(Percent)/100.0 + 3.1415926/2);
		sy = y- 30 * sin(-2*3.1415926*float(Percent)/100.0 + 3.1415926/2);
...
void __fastcall TFormCalibrate::FormShow(TObject *Sender)
{
	...
	if(ci->xPoints < 2) ci->xPoints = 2;
	else if(ci->xPoints > 6) ci->xPoints = 6;

	if(ci->yPoints < 2) ci->yPoints = 2;
	else if(ci->yPoints > 6) ci->yPoints = 6;
	...
	for(uint8_t i=0; i<ci->xPoints; i+=1)
	{
		...
		if(i<ci->xPoints-1 && FormConfig->CheckBoxCalibAuxPoint->Checked == true)
		{
			x += (xStep/2);
...
void __fastcall TFormCalibrate::CalibrateFSM(void)
{
	...
	else
	{
		...
		while(InPtr != OutPtr)
		{
			...
			if(cp->Points != 1)	continue;       //single point
