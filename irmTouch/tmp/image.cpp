...
void __fastcall TFormPoints::DrawPoints(int Points, point_t (&pt)[])
{
	...
	if(SrcBitmap->Width != Image1->Width || SrcBitmap->Height != Image1->Height) {
	...
	xMgLeft = SrcBitmap->Width * X_MARGIN_LEFT_SCALE / (MAX_X_COOD - MIN_X_COOD);
	xMgRight = SrcBitmap->Width * X_MARGIN_RIGHT_SCALE / (MAX_X_COOD - MIN_X_COOD);
	yMgUp = SrcBitmap->Width * Y_MARGIN_UP_SCALE / (MAX_Y_COOD - MIN_Y_COOD);
	yMgDown = SrcBitmap->Width * Y_MARGIN_DOWN_SCALE / (MAX_Y_COOD - MIN_Y_COOD);
	...
	for(int i=0; i<Points; i+=1) {
		...
		if(pt[i].Status == AREA_MOVE || pt[i].Status == AREA_UPED)
		{
			...
			x1 = x1 * Ratio_ + XOff_ * SCALE * TouchAreaWidth * Ratio_/MAX_X_SCALE;
			x2 = x2 * Ratio_ + XOff_ * SCALE * TouchAreaWidth * Ratio_/MAX_X_SCALE;

			y1 = y1*TouchAreaHeight/32768 + yMgDown;
			y2 = y2*TouchAreaHeight/32768 + yMgDown;

			Msg.cat_printf(L"%.3d:(%4d,%4d)", pt[i].ID, (x1+x2)/2, (y1+y2)/2);
			...
			if(DrawLine->Checked == true) {
				...
				if(LastPoint[i].Status == AREA_MOVE) {
					...
					Canvas->Pen->Color = PointCl[pt[i].ID%(sizeof(PointCl)/sizeof(TColor))];
					Canvas->MoveTo(ox, oy);
				}
			}
			...
			if(DrawLine->Checked == false) {
				if(pt[i].Status == AREA_MOVE) {
					Canvas->Brush->Color = PointCl[pt[i].ID%(sizeof(PointCl)/sizeof(TColor))];
					Canvas->Ellipse(x1, Flip_Y(y1), x2, Flip_Y(y2));
				} else {
					Canvas->Brush->Color = clWhite;

					//if(FixSize1->Checked == true)
					Canvas->Rectangle(x1, Flip_Y(y1), x2, Flip_Y(y2));//((x1+x2)/2-10, (y1+y2)/2-10, (x1+x2)/2+10, (y1+y2)/2+10);
				}
			} else {
				if(LastPoint[i].Status == AREA_MOVE && (pt[i].Status == AREA_MOVE || pt[i].Status == AREA_UPED))
				{
					int nx, ny;
...
void __fastcall TFormPoints::FormMouseWheelDown(TObject *Sender, TShiftState Shift,
...
void __fastcall TFormPoints::FormMouseWheelUp(TObject *Sender, TShiftState Shift,
          TPoint &MousePos, bool &Handled)
{
	if(Ratio_ < 20) {
		Ratio_ += 1;
...
void __fastcall TFormPoints::Image1MouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TFormPoints::Image1MouseMove(TObject *Sender, TShiftState Shift, int X,
          int Y)
{
    if(MouseIsDown) {
...
void __fastcall TFormPoints::Image1MouseUp(TObject *Sender, TMouseButton Button, TShiftState Shift,
          int X, int Y)
{
    MouseIsDown = 0;
}
